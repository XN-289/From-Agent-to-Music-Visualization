...
import { importFolder, importLocalFiles, resyncAllFolders, LOCAL_MUSIC_SCAN_PROGRESS_EVENT } from '../services/localMusicService';
...
export const Grid3D: React.FC<Grid3DProps> = (props) => {
    ...
    const handleFolderImport = async () => {
        ...
        if (!availability.supported) {
            ...
            input.type = 'file';
            input.multiple = true;
            input.accept = '.mp3,.flac,.m4a,.wav,.ogg,.opus,.aac,.lrc,.vtt,.ttml,.qrc,.yrc,.krc';
            ...
            input.addEventListener('change', async () => {
                ...
                try {
                    const importedSongs = await importLocalFiles(input.files);
                    if (importedSongs.length > 0) {
                        await onRefreshLocalSongs();
                    }
                } catch (error) {
                    console.error('[Grid3D] Failed to import selected local files:', error);
