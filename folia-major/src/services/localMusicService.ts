...
function createReadableFileHandle(file: File): FileSystemFileHandle {
    return {
        kind: 'file',
        name: file.name,
        getFile: async () => file,
...
// Fallback import for browsers without the File System Access API. This lets
// the web build import individual audio and sidecar lyric files without
// showDirectoryPicker() while keeping the same processing path as folder scans.
export async function importLocalFiles(files: FileList | File[]): Promise<LocalSong[]> {
    const selectedFiles = Array.from(files);
    const audioFiles = selectedFiles.filter(file => isAudioFile(file));
    if (audioFiles.length === 0) {
        console.warn('[LocalMusic][Import] No audio files found in selection.');
        return [];
    }

    const lrcMap = new Map<string, LocalLyricFileCandidate>();
    const tlrcMap = new Map<string, File>();

    const folderName = '本地文件';
    for (const file of selectedFiles) {
        if (TRANSLATION_LYRIC_EXTENSIONS.test(file.name)) {
            const baseName = getSidecarLyricBasePath(file.name, 'translationLyric');
            tlrcMap.set(`${folderName}/${baseName}`, file);
            continue;
        }

        if (LYRIC_EXTENSIONS.test(file.name)) {
            const baseName = getSidecarLyricBasePath(file.name, 'lyric');
            lrcMap.set(`${folderName}/${baseName}`, {
                file,
                format: resolveExplicitFileTimedLyricFormat(file.name),
            });
        }
    }
    ...
    for (const file of audioFiles) {
        const entry: FileEntryForImport = {
            handle: createReadableFileHandle(file),
            file,
            folderName,
        ...
        try {
            const { song } = await buildImportedSong(
                entry,
                lrcMap,
                tlrcMap,
                new Map(),
            ...
            console.error(`[LocalMusic][Import] Failed to import "${file.name}":`, error);
        }
    }

    if (importedSongs.length > 0) {
        await saveLocalSongs(importedSongs);
        notifyLocalMusicUpdated();
    }

    console.log(`[LocalMusic][Import] Imported ${importedSongs.length} files without directory handles.`);
