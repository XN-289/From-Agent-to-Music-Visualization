...
- **音乐生成**：SunoProvider 兼容层（`src/lib/providers/`）——Mock（本地合成，离线可演示）、sunoapi.org 第三方 API、公司统一音乐代理（musicproxy）可切换
...
FOLIA_STAGE_TOKEN=xxx    # 在 Folia 桌面端 Stage Mode 中复制
...
/api/chat ──→ pi AgentSession（pi-agent-core）   /api/jobs/[id] ──→ SunoProvider.getJob
   ...
   ▼
SunoProvider 兼容层 ── Mock（本地合成 WAV）| SunoApi（sunoapi.org）| MusicProxy（公司网关）
...
| `POST /api/songs/[id]/push-folia` | 把已完成歌曲落盘并推送到本机 Folia Stage |
